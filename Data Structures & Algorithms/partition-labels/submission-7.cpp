...
public:
    vector<int> partitionLabels(string s) {
        ...
        vector<int> result;
        int start = 0; //too compute the size of the subarray
        int maxEnd = 0;
        for (int i = 0; i < s.length(); i++){
            maxEnd = max(maxEnd, lastIndex[s[i]]); //end between the current and the end of the character
            if (i == maxEnd){
                result.push_back(maxEnd - start + 1); //add the size of the subarray that has the last index of a character possible
                start = maxEnd + 1; //set the size = 0 to create a new subarray
            }
        }
        return result;
    }
